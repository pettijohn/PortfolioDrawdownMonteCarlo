...
self.onmessage = async (event: MessageEvent<SimulationWorkerRequest>) => {
    ...
    try {
        ...
        const results = await runGoSimulation(message.requestId, message.config);
        postMessageToApp({
            type: "result",
            requestId: message.requestId,
            results,
...
async function runGoSimulation(
    requestId: number,
    ...
    const shards: GoSimulationShard[] = [];
    let completedShards = 0;

    postMessageToApp({
        type: "state",
        requestId,
        state: `Running Go WASM with ${workerCount} worker${workerCount === 1 ? "" : "s"}`,
    });

    await Promise.all(shardRounds.map((simulationRounds, shardIndex) => {
        return new Promise<void>((resolve, reject) => {
            const worker = new Worker("./goMonteCarloShardWorker.js", { type: "module" });

            worker.onmessage = (event: MessageEvent<GoShardMessage>) => {
                const message = event.data;
                ...
                if (message.type === "shardError") {
                    reject(new Error(message.message ?? "Go WASM shard failed"));
                    return;
                }

                if (!message.shard) {
                    reject(new Error("Go WASM shard returned no data"));
                    return;
                }
                ...
                postMessageToApp({
                    type: "state",
                    requestId,
                    state: `Running Go WASM shard ${completedShards} of ${workerCount}`,
                });
                resolve();
            };
...
async function analyzeShards(
    ...
    postMessageToApp({
        type: "state",
        requestId,
        state: `Analyzing Go WASM with ${analysisWorkerCount} workers`,
    });
    ...
    await runAnalysisPool(
        ...
        (year, result) => {
            ...
            postMessageToApp({
                type: "state",
                requestId,
                state: `Analyzing Go WASM year ${completedYears} of ${config.simulationYears}`,
            });
        },
    );
        ...
        for (let workerIndex = 0; workerIndex < workerCount; workerIndex++) {
            ...
            worker.onmessage = (event: MessageEvent<AnalysisMessage>) => {
                ...
                if (message.type === "analysisError") {
                    failed = true;
                    terminateWorkers();
                    reject(new Error(message.message ?? "Go WASM analysis failed"));
                    return;
                }
                ...
                if (!message.result) {
                    failed = true;
                    terminateWorkers();
                    reject(new Error("Go WASM analysis returned no result"));
                    return;
                }
