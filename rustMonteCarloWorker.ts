...
type RustShardMessage = {
    type: "shardResult" | "shardError";
    requestId: number;
    shardIndex: number;
    shard?: RustSimulationShard;
    message?: string;
};

type RustAnalysisMessage = {
...
async function runRustSimulation(
    requestId: number,
    ...
    const shards: RustSimulationShard[] = [];
    ...
    await Promise.all(shardRounds.map((simulationRounds, shardIndex) => {
        return new Promise<void>((resolve, reject) => {
            const worker = new Worker("./rustMonteCarloShardWorker.js", { type: "module" });

            worker.onmessage = (event: MessageEvent<RustShardMessage>) => {
                const message = event.data;
...
function runAnalysisPool(
    ...
    return new Promise((resolve, reject) => {
        ...
        for (let workerIndex = 0; workerIndex < workerCount; workerIndex++) {
            ...
            worker.onmessage = (event: MessageEvent<RustAnalysisMessage>) => {
                const message = event.data;
